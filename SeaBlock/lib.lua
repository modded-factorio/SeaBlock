...
local function add_recipe_unlock(technology, recipe, insertindex)
  ...
  if addit then
    ...
    if insertindex then
      table.insert(technology.effects, insertindex, {
        type = "unlock-recipe",
        recipe = recipe
      })
    else
...
function seablock.lib.remove_effect(
technology_name,
  effect_type,
  effect_key,
  effect_value
...
--]]
function seablock.reskins.composite_existing_icons(
target_name,
  target_type,
  icons
)
  ...
  seablock.reskins.assign_icons(target_name, {
    type = target_type,
    icon = composite_icon
  })
end
