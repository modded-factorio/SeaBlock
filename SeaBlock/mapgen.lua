...
distance,
  ...
  p = p * probability
  p =
    noise.random_penalty(p, probability * 0.5, {
...
data.raw.turret["small-worm-turret"].autoplace =
  worm_autoplace(0, 1, "z", true, "enemy-base")
data.raw.turret["medium-worm-turret"].autoplace =
  worm_autoplace(1, 1, "y", true, "enemy-base")
if data.raw.turret["bob-big-explosive-worm-turret"] then
  data.raw.turret["bob-big-explosive-worm-turret"].autoplace =
    worm_autoplace(1.5, 0.5, "v", false, "enemy-base")
end
if data.raw.turret["bob-big-fire-worm-turret"] then
  data.raw.turret["bob-big-fire-worm-turret"].autoplace =
    worm_autoplace(1.5, 0.5, "v", false, "enemy-base")
end
if data.raw.turret["bob-big-poison-worm-turret"] then
  data.raw.turret["bob-big-poison-worm-turret"].autoplace =
    worm_autoplace(1.5, 0.5, "v", false, "enemy-base")
end
if data.raw.turret["bob-big-piercing-worm-turret"] then
  data.raw.turret["bob-big-piercing-worm-turret"].autoplace =
    worm_autoplace(1.5, 0.5, "v", false, "enemy-base")
end
if data.raw.turret["bob-big-electric-worm-turret"] then
  data.raw.turret["bob-big-electric-worm-turret"].autoplace =
    worm_autoplace(1.5, 0.5, "v", false, "enemy-base")
end
if data.raw.turret["bob-giant-worm-turret"] then
  data.raw.turret["bob-giant-worm-turret"].autoplace =
    worm_autoplace(2, 0.6, "u", false, "enemy-base")
end
if data.raw.turret["behemoth-worm-turret"] then
  data.raw.turret["big-worm-turret"].autoplace =
    worm_autoplace(1.5, 0.5, "v", false, "enemy-base")
  data.raw.turret["behemoth-worm-turret"].autoplace =
    worm_autoplace(2, 0.2, "t", false, "enemy-base")
else
  data.raw.turret["big-worm-turret"].autoplace =
    worm_autoplace(2, 1, "v", false, "enemy-base")
end
...
= noise.define_noise_function(function(x, y, tile, map) -- No cliffs in starting area
  local t =
    noise.clamp(
      (tile.tier - 0.2) * noise.ceil(
        noise.var("control-setting:cliffs:richness:multiplier")
      ),
      0,
      1
    )
...
= noise.define_noise_function(function(x, y, tile, map) -- Increase gradient for cliffs while leaving waterline unchanged
  x = x + 40000
  y = y
  local v = make_basis_noise_function(map.seed, 5, 6, 1 / 64)(x, y)
  v = noise.max(v, 0)
  v = (v * elevation_scale) - (waterline * (elevation_scale - 1))
  return v
end)