...
script.on_event(defines.events.on_chunk_generated, function(e)
  ...
  if surface.name ~= "nauvis" and surface.name:sub(
    1,
    14
  ) ~= "battle_surface" then return end
...
script.on_event(defines.events.on_player_main_inventory_changed, function(e)
  ...
  if not inv then
    return -- Compatibility with BlueprintLab_Bud17
  end
  for k, v in pairs(global.unlocks) do
    for _, v2 in ipairs(v) do
      if player.force.technologies[v2] and not player.force.technologies[v2].researched and inv.get_item_count(
        k
      ) > 0 then
