...
-- So closer to 80% sulfur return
  ...
  working_sound = {
    sound = {
      filename = "__angelsrefining__/sound/ore-sorting-facility.ogg",
      volume = 0.5
    },
    idle_sound = {
...
bobmods.lib.tech.add_recipe_unlock(
  "oil-gas-extraction",
  "sb-blue-algae-liquefaction"
)

-- Setup recipe bases

local min_water_icons =
  angelsmods.functions.create_viscous_liquid_fluid_icon(nil, {
    {039, 112, 194},
    {093, 067, 049},
    {070, 133, 232},
    {109, 070, 020, 0.8}
...
local slag_recipe_shifts = {{-10, 10}, {10, 10}}
...
for name, base_icons in pairs(slag_processing_list) do
  ...
  if recipe then
    ...
    if recipe_results[1].name ~= "angels-void" then
      ...
      -- Setup base layer
      composite_recipes[name] = {
        base = {icons = base_icons}
      }

      -- Build icon overlays based on recipe products
      for _, product in pairs(recipe_results) do
        composite_recipes[name][product.name] = {
          shift = slag_recipe_shifts[shift_index],
          scale = 0.5
        }
